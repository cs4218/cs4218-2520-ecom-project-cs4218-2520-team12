...
- David Vicedo (A0273234J)
    - Integration Tests:
        - Product Details Integration Test
            - `client/src/__tests__/integration/product-details.int.test.js`
        - Search Flow and Context State Management Integration Test
            - `client/src/__tests__/integration/search-flow.int.test.js`
        - Product Discovery Routes Integration Test
            - `controllers/__tests__/product-discovery.routes.int.test.js`
    - UI Tests (Playwright):
        - Browse to Product Details End-to-End Test
            - `ui/browse-product.spec.ts`
        - Search Flow End-to-End Test (Success and Empty Results)
            - `ui/search.spec.ts`
    - CI/CD:
...
- Wong An Wei (A0273528X)
    - Volume Testing
        - Volume Testing for Protected Routes (Unauthenticated Rejection + Authenticated Access)
            - `performance-testing/k6/volume/protected-routes-volume.js`
        - Volume Testing for Orders (User Order History + Order Creation Flow)
            - `performance-testing/k6/volume/orders-volume.js`
        - Volume Testing for Profile (Fetch + Update)
            - `performance-testing/k6/volume/profile-volume.js`
        - Volume Testing for Admin View Users (Admin Route Protection)
            - `performance-testing/k6/volume/admin-users-volume.js`
        - Volume Test Runner and Documentation
